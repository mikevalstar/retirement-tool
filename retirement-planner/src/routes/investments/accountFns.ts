...
export const updateAccount = createServerFn({ method: 'POST' })
  .inputValidator(
    (data: { id: number; name: string; type: AccountType; ownerId: number }) => data,
  )
  .handler(async ({ data }) => {
...
export const createSnapshot = createServerFn({ method: 'POST' })
  .inputValidator(
    (data: { accountId: number; date: string; balance: number; note?: string }) => data,
  )
  .handler(async ({ data }) => {
...
export const createBulkSnapshots = createServerFn({ method: 'POST' })
  .inputValidator(
    (data: { date: string; entries: Array<{ accountId: number; balance: number }> }) =>
      data,
  )
  .handler(async ({ data }) => {
    for (const entry of data.entries) {
      await createSnapshot({ data: { accountId: entry.accountId, date: data.date, balance: entry.balance } })
    }
  })
