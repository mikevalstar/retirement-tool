...
export const Route = createFileRoute("/blog/$slug")({
  loader: ({ params }) => {
    const post = Array.from(
      new Map([...allBlogs].sort((a, b) => new Date(b.pubDate).valueOf() - new Date(a.pubDate).valueOf()).map((entry) => [entry.slug, entry])).values(),
  ...
  return (
    <main className="page-wrap px-4 pb-12 pt-16">
      <article className="island-shell rounded-2xl p-6 sm:p-8">
        {post.heroImage ? <img src={post.heroImage} alt="" className="mb-6 h-64 w-full rounded-2xl object-cover" /> : null}
        ...
        <p className="mb-6 text-sm text-[var(--sea-ink-soft)]">{new Date(post.pubDate).toLocaleDateString()}</p>
        <div className="prose prose-slate prose-headings:text-[var(--sea-ink)] prose-p:text-[var(--sea-ink-soft)] prose-li:text-[var(--sea-ink-soft)] prose-ul:text-[var(--sea-ink-soft)] prose-ol:text-[var(--sea-ink-soft)] prose-strong:text-[var(--sea-ink)] prose-a:text-[var(--lagoon-deep)] max-w-none">
          {post.mdx ? <MDXContent code={post.mdx} components={{ MdxCallout, MdxMetrics }} /> : <div dangerouslySetInnerHTML={{ __html: post.html ?? "" }} />}
        </div>
      </article>
    </main>
  );
