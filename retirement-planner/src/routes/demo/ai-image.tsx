...
function ImagePage() {
  const [prompt, setPrompt] = useState("A cute baby sea otter wearing a beret and glasses, sitting at a small cafe table, sipping a cappuccino");
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {images.map((image, index) => (
                    <div key={index} className="relative group">
                      <img src={getImageSrc(image)} alt={`Generated image ${index + 1}`} className="w-full rounded-lg border border-gray-700" />
                      <button
                      ...
                      {image.revisedPrompt && <p className="mt-2 text-xs text-gray-400 italic">Revised: {image.revisedPrompt}</p>}
                    </div>
                  ))}
                </div>
              </div>
