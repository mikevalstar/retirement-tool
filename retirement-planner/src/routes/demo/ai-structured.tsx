	...
	return (
		<div className="space-y-6">
			...
			<div className="flex flex-wrap gap-4">
				...
				<div className={`flex items-center gap-2 px-2 py-1 rounded-full ${difficultyColors[recipe.difficulty]}`}>
				...
				<div>
					...
					<div className="flex flex-wrap gap-4 text-sm">
						{recipe.nutritionPerServing.calories && (
							<span className="px-3 py-1 bg-gray-700 rounded-full text-gray-300">{recipe.nutritionPerServing.calories} cal</span>
						)}
						{recipe.nutritionPerServing.protein && (
							<span className="px-3 py-1 bg-gray-700 rounded-full text-gray-300">Protein: {recipe.nutritionPerServing.protein}</span>
						)}
						{recipe.nutritionPerServing.carbs && (
							<span className="px-3 py-1 bg-gray-700 rounded-full text-gray-300">Carbs: {recipe.nutritionPerServing.carbs}</span>
						)}
						{recipe.nutritionPerServing.fat && <span className="px-3 py-1 bg-gray-700 rounded-full text-gray-300">Fat: {recipe.nutritionPerServing.fat}</span>}
					</div>
				</div>
			)}
		</div>
		...
		<div className="min-h-[calc(100vh-80px)] bg-gray-900 p-6">
			<div className="max-w-6xl mx-auto">
				...
				<p className="text-gray-400 mb-6">
					Compare two output modes: <strong className="text-orange-400">One-Shot</strong> returns freeform markdown, while{" "}
					<strong className="text-orange-400">Structured</strong> returns validated JSON conforming to a Zod schema.
				</p>
				...
				<div className="mt-5 lg:col-span-2 bg-gray-800 rounded-lg p-6 border border-orange-500/20">
						...
						{result && (
							<span
								className={`px-2 py-1 rounded text-xs font-medium ${
									result.mode === "structured" ? "bg-purple-500/20 text-purple-400" : "bg-blue-500/20 text-blue-400"
								}`}>
					...
					{error && <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400 mb-4">{error}</div>}
