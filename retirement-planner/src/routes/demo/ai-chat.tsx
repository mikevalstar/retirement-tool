    ...
    <div ref={messagesContainerRef} className="flex-1 overflow-y-auto pb-4 min-h-0">
      <div className="max-w-3xl mx-auto w-full px-4">
        {messages.map((message) => {
          ...
          return (
            <div key={message.id} className={`p-4 ${message.role === "assistant" ? "bg-linear-to-r from-orange-500/5 to-red-600/5" : "bg-transparent"}`}>
              <div className="flex items-start gap-4 max-w-3xl mx-auto w-full">
                {message.role === "assistant" ? (
                  ...
                  <div className="w-8 h-8 rounded-lg bg-gray-700 flex items-center justify-center text-sm font-medium text-white flex-shrink-0">Y</div>
                )}
                <div className="flex-1 min-w-0">
                  {message.parts.map((part, index) => {
                    if (part.type === "text" && part.content) {
                      return (
                        <div className="flex-1 min-w-0 prose dark:prose-invert max-w-none prose-sm" key={index}>
              ...
              <div className="relative max-w-xl mx-auto flex items-center gap-2">
                <button
                  ...
                  className={`p-3 rounded-lg transition-colors ${
                    isRecording ? "bg-red-600 hover:bg-red-700 text-white" : "bg-gray-800/50 text-gray-400 hover:text-orange-400 border border-orange-500/20"
                  } disabled:opacity-50`}
